...
const userSlice = createSlice({
    ...
    reducers: {
        login: (state, action) => {
            ...
            if (typeof window !== 'undefined') {
                try {
                    ...
                    let existing = []
                    try {
                        const rawExisting = localStorage.getItem('gocart_users')
                    ...
                    let deletedIds = []
                    try {
                        const rawDeleted = localStorage.getItem('gocart_deleted_user_ids')
                        deletedIds = rawDeleted ? JSON.parse(rawDeleted) : []
                        if (!Array.isArray(deletedIds)) deletedIds = []
                    } catch { deletedIds = [] }

                    const withoutDupe = existing.filter(u => u && u.id !== user.id && !deletedIds.includes(u.id))
                    ...
                    if (!Array.isArray(state.savedUsers)) state.savedUsers = []
        ...
        register: (state, action) => {
            ...
            if (typeof window !== 'undefined') {
                try {
                    ...
                    const withoutDupe = existing.filter(u => u && u.id !== newUser.id && !deletedIds.includes(u.id))
        ...
        addUserAddress: (state, action) => {
            ...
            if (state.currentUser) {
                ...
                if (action.payload.isDefault) {
                    state.currentUser.addresses.forEach(a => { if (a) a.isDefault = false })
                }
        ...
        updateUserAddress: (state, action) => {
            if (!action.payload?.id || !action.payload.updatedData) return
        ...
        setDefaultUserAddress: (state, action) => {
            ...
            if (state.currentUser && Array.isArray(state.currentUser.addresses)) {
                state.currentUser.addresses.forEach(a => {
                    if (a) a.isDefault = a.id === action.payload
                })
                if (!Array.isArray(state.savedUsers)) state.savedUsers = []
                const userIndex = state.savedUsers.findIndex(u => u?.id === state.currentUser.id)
                if (userIndex !== -1) {
