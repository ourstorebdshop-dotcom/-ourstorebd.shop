...
const headerFooterSlice = createSlice({
    ...
    reducers: {
        ...
        addHeaderNavLink: (state, action) => {
            if (!action.payload) return
            if (!Array.isArray(state.header.navLinks)) state.header.navLinks = []
            const newLink = {
        ...
        addFooterSocialLink: (state, action) => {
            if (!action.payload) return
            if (!Array.isArray(state.footer.social.links)) state.footer.social.links = []
            const newSocial = {
        ...
        addFooterSection: (state, action) => {
            if (!action.payload) return
            if (!Array.isArray(state.footer.sections)) state.footer.sections = []
            const newSection = {
        ...
        addFooterSectionLink: (state, action) => {
            if (!action.payload?.sectionId || !action.payload?.link) return
        ...
        updateFooterSectionLink: (state, action) => {
            if (!action.payload?.sectionId || !action.payload?.linkId) return
            const { sectionId, linkId, updates } = action.payload
            const sec = state.footer.sections.find(s => s.id === sectionId)
            if (sec && Array.isArray(sec.links)) {
