...
const fraudSlice = createSlice({
    ...
    reducers: {
        hydrateFraud: (state, action) => {
            ...
            state.settings = data.settings || (data.maxOrdersPerPhonePerHour ? data : null)
        ...
        unblockPhone: (state, action) => {
            ...
            state.blockedPhones = state.blockedPhones.filter(p => p !== target && p.replace(/^\+88/, '') !== String(target || '').replace(/^\+88/, ''))
        },
        ...
        removeFromWatchlist: (state, action) => {
            const target = action.payload
            state.watchlist = state.watchlist.filter(w => w.phone !== target && String(w.phone || '').replace(/^\+88/, '') !== String(target || '').replace(/^\+88/, ''))
        },
        ...
        removeAuditLog: (state, action) => {
            state.auditLogs = (state.auditLogs || []).filter(l => l.id !== action.payload)
        },
