...
const bannerSlice = createSlice({
    ...
    reducers: {
        ...
        addBanner: (state, action) => {
            ...
            const newBanner = {
                ...action.payload,
                id: action.payload.id || `banner_${Date.now()}`,
                priority: action.payload.priority ?? (state.banners.length + 1),
                createdAt: action.payload.createdAt || new Date().toISOString(),
            }
            state.banners.unshift(newBanner)
        },
        updateBanner: (state, action) => {
            if (!action.payload?.id || !Array.isArray(state.banners)) return
        ...
        reorderBanners: (state, action) => {
            ...
            if (fromIndex < 0 || fromIndex >= state.banners.length) return
            if (toIndex < 0 || toIndex >= state.banners.length) return
            const [moved] = state.banners.splice(fromIndex, 1)
            if (moved) {
                state.banners.splice(toIndex, 0, moved)
                state.banners.forEach((b, i) => { if (b) b.priority = i + 1 })
            }
        },
        resetBanners: (state) => {
