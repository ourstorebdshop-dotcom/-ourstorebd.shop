...
import { serverSaveDoc as saveDocToFirestore } from '../firestoreServer.js'
...
function isFirebaseConfigured() { return Boolean(adminDb) }
...
export async function getStoredIntegrationsSettings() {
    if (!adminDb) return defaultIntegrationsSettings
    try {
        ...
        console.warn('[SyncEngine] Failed to read integrations settings via Admin SDK:', err)
    }
...
export async function recordSyncEvent(logEntry, currentSettings) {
    ...
    try {
        ...
        if (logEntry.status === 'SUCCESS') {
            ...
            // If this was a successful re-sync of a previously failed order, decrement failedCount
            const hadFailure = (settings.syncLogs || []).some(
                l => l.orderId === logEntry.orderId && l.service === logEntry.service && l.status === 'FAILED'
            )
            if (hadFailure && (stats.failedCount || 0) > 0) {
                stats.failedCount = Math.max(0, stats.failedCount - 1)
            }
