...
export async function sendTelegramOrderNotification(order, settings = {}, origin = '', options = {}) {
    ...
    try {
        ...
        // FALLBACK 1: If Telegram rejects due to inline keyboard/button URL issues, retry without buttons
        if (!res.ok && data?.description && (
            data.description.includes('inline keyboard') ||
            data.description.includes('button') ||
            data.description.includes('reply_markup') ||
            data.description.includes('URL')
        )) {
            console.warn('[Telegram] Inline keyboard rejected by Telegram. Retrying without buttons...')
            const btnFallbackController = new AbortController()
            const btnFallbackTimeoutId = setTimeout(() => btnFallbackController.abort(), 6500)
            ...
            res = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
                ...
                signal: btnFallbackController.signal,
            })
            clearTimeout(btnFallbackTimeoutId)
            data = await res.json().catch(() => ({}))
        }

        // FALLBACK 2: If HTML parse error occurred, retry immediately with plain text (and no buttons)
        if (!res.ok && data?.description && (
