...
export async function saveCollectionToFirestore(collectionName, items) {
    if (!isFirebaseConfigured() || !Array.isArray(items)) return false
    try {
        const promises = items.map(item => {
            const docId = String(item.id || item.code || item._docId || `doc_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`)
 ...
 * @param {string} collectionName
 * @param {Array} items - current items (each must have 'id' or 'code')
 * @returns {Promise<boolean>}
 */
