...
export function getFraudConfig(adminOverrides = null) {
    ...
    // Unwrap nested settings if passed as the entire fraud doc
    const effectiveOverrides = adminOverrides.settings ? { ...adminOverrides.settings, ...adminOverrides } : adminOverrides
    const rawThresholds = effectiveOverrides.riskThresholds || {}
    const low = rawThresholds.low ?? rawThresholds.MEDIUM ?? rawThresholds.medium_review ?? FRAUD_DEFAULTS.riskThresholds.low
    const medium = rawThresholds.medium ?? rawThresholds.HIGH ?? rawThresholds.high_block ?? FRAUD_DEFAULTS.riskThresholds.medium
    ...
    return {
        ...
        riskWeights: {
            ...
            ...(effectiveOverrides.riskWeights || {}),
        },
    }
}
