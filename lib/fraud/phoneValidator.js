...
export function validateBDPhone(phone) {
    ...
    // Check for obviously fake numbers (8 identical digits in subscriber portion or 9 identical after 01)
    const subTail = local.slice(3)
    const allSameSub = subTail.length === 8 && subTail.split('').every(d => d === subTail[0])
    const tail2 = local.slice(2)
    const allSameTail2 = tail2.split('').every(d => d === tail2[0])
    if (allSameSub || allSameTail2) {
        return {
            valid: false,
            normalized: '',
            local,
    ...
    // Sequential ascending/descending check (e.g. 01712345678, 01787654321, 01345678901)
    const isSequential = subTail === '12345678' || subTail === '87654321' || tail2 === '123456789' || tail2 === '234567890' || tail2 === '345678901' || tail2 === '987654321'
    if (isSequential) {
        return {
            valid: false,
            normalized: '',
