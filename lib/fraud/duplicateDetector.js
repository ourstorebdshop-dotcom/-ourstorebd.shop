...
export async function getOrdersForPhone(phone) {
    ...
    try {
        const qNormalized = adminDb.collection('orders').where('address.normalizedPhone', '==', normalizedPhone).get()

        const rawPhone = String(phone).trim()
        const queries = [qNormalized]
        if (rawPhone && rawPhone !== normalizedPhone) {
            queries.push(adminDb.collection('orders').where('address.phone', '==', rawPhone).get().catch(() => null))
        }
        ...
        console.error('[FraudDetector] Failed to fetch orders for phone via Admin SDK:', error)
        return []
    }
}
