...
import { collection, addDoc, getDocs, query, orderBy, limit as fbLimit, deleteDoc, doc, writeBatch } from 'firebase/firestore'
...
export async function deleteAuditLog(logId) {
    if (!isFirebaseConfigured() || !logId) return false

    try {
        await deleteDoc(doc(db, 'fraud_audit_log', String(logId)))
        return true
    } catch (error) {
        console.error('[AuditLog] Failed to delete audit log entry:', error)
        return false
    }
}
...
export async function clearAllAuditLogs() {
    ...
    try {
        ...
        console.error('[AuditLog] Failed to clear all audit logs:', error)
        return false
    }
}
