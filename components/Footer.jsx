...
// Clean SVG Icons for Contact Items
const MailIcon = () => (
...
// Social Icons Component Mapping
const SocialIconRenderer = ({ platform }) => {
        ...
        case 'linkedin':
            return (
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path d="M5.0013 7.50033H1.66797V17.5003H5.0013V7.50033Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    <path d="M3.33464 5.00033C4.25511 5.00033 5.0013 4.25413 5.0013 3.33366C5.0013 2.41318 4.25511 1.66699 3.33464 1.66699C2.41416 1.66699 1.66797 2.41318 1.66797 3.33366C1.66797 4.25413 2.41416 5.00033 3.33464 5.00033Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
            );
        case 'youtube':
            return (
...
const Footer = () => {
    ...
    const showSocial = social.showSocial !== false;
    ...
    const showBottomBar = bottomBar.showBottomBar !== false;
    const copyrightRaw = bottomBar.copyrightText || 'Copyright {year} © Our Store BD. All Rights Reserved. Made for Bangladesh 🇧🇩';
    const copyrightText = copyrightRaw.replace('{year}', new Date().getFullYear());
    const showBadges = bottomBar.showBadges !== false;
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                            ...
                            {showSocial && socialLinks.length > 0 && (
                                <div className="flex items-center gap-3 mt-6">
                                    {socialLinks.map((item) => (
                                        ...
                                        >
                                            <SocialIconRenderer platform={item.platform} />
                                        </a>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="flex flex-wrap justify-between sm:justify-end gap-10 sm:gap-16 w-full md:w-auto text-xs sm:text-sm">
                        {sections.map((section) => (
                            <div key={section.id} className="min-w-[130px]">
                                <h3 className="font-bold text-slate-800 md:mb-4 mb-3 tracking-wide text-xs uppercase">
                                    {section.title}
                                </h3>
                ...
                {showBottomBar && (
                    <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
                        <p>{copyrightText}</p>
                        {showBadges && badgesText && (
                            <p className="text-slate-400">{badgesText}</p>
                        )}
                    </div>
                )}
            </div>
